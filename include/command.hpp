...
private:
    ...
    const char taskTable[TaskBit::taskButtom][SortBit::sortButtom][COMMAND_THREAD_NAME_LENGTH] =
    {
        {
        ...
        {
            "s_bub", "s_sec", "s_ins", "s_she", "s_mer",
            "s_qui", "s_hea", "s_cou", "s_buc", "s_rad"
        }
    };
    //#pragma pack(8)
    struct TaskPlan
    ...
    void runOptimum();
    void getOptimumResult(const Expression &express, const double leftEndpoint,
        const double rightEndpoint, const double epsilon) const;
    void setOptimumBit(char *const argv[]);
    void runIntegral();
    void getIntegralResult(const Expression &express, const double lowerLimit,

...
void printFile(const char *const pathname, const bool reverse = false,
...
#define LOGGER(level, format, args...) \
    logger.outputLog(level, __FILE__, __LINE__, format, ##args)
...
public:
    ...
    void outputLog(const uint32_t level, const std::string &codeFile, const uint32_t codeLine,
...
template<typename... Args>
void Log::outputLog(const uint32_t level, const std::string &codeFile, const uint32_t codeLine,
    const char *const format, const Args... args)
{
    if (level >= minLevel)
    {

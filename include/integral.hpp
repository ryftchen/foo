...
#define TRAPEZOIDAL_MIN_STEP    pow(2, 3)
...
private:
    ...
    friend double trapezoid(const Expression &express, const double left, const double h,
        const uint32_t step);
};
...
private:
    const Expression &fun;
    double simpsonIntegral(const double left, const double right, const double eps) const;
    double compositeSimpsonOneThird(const double left, const double right, const uint32_t n) const;
    double simpsonOneThird(const double left, const double right) const;
};
...
private:
    const Expression &fun;
    double sampleFromUniformDistribution(const double lower, const double upper,
        const double eps) const;
#ifdef NO_UNIFORM
    double sampleFromNormalDistribution(const double lower, const double upper,
        const double eps) const;
#endif
};

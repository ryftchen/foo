...
template <typename T>
concept mappingSubCLI = std::is_same_v<T, app_dp::ApplyDesignPattern>;
//! @brief Constraint for mapping sub-cli category.
//! @tparam T - type of mapping sub-cli category
template <typename T>
concept mappingSubCLICat = std::is_same_v<T, app_dp::BehavioralInstance>
    || std::is_same_v<T, app_dp::CreationalInstance> || std::is_same_v<T, app_dp::StructuralInstance>;
